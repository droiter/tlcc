...
Convention: r[i, j, k] = Pearson corr( close_i[t], close_j[t+k] )
over overlapping days, k in 0..K  -> i LEADS j by k days when k>0.
Data: raw close prices (user's choice), lag window single-sided 0..+20.
"""
import sys
import numpy as np
import pandas as pd
...
OUTP = f'{OUTDIR}/tlcc_tlc_k{K}'

# ---- index name map (from indices_report.tsv) ----
names = {}
try:
...
# ---- load wide close panel: index (date) x columns (exchange, code) ----
print('loading...', flush=True)
df = pd.read_hdf(SRC, 'nfq')  # 625494 x 6, MultiIndex (date, exchange, code)
wide = df['close'].unstack(['exchange', 'code'])  # date x (ex, code)
wide = wide.sort_index()
Xcols = list(wide.columns)          # [(exchange, code), ...]
m = len(Xcols)
X = wide.to_numpy(dtype=np.float64)  # n x m, NaN outside a series' own span
dates = wide.index
...
print(f'panel {X.shape[0]} dates x {m} indices', flush=True)

# ---- pairwise aligned (NaN-dropped) TLC ----
def tlc_series(x, y, K):
    """x,y same length, no NaN. returns r[0..K] with r[k]=pearson(x[:L-k], y[k:])."""
    L = len(x)
    out = np.full(K + 1, np.nan)
    kk = min(K, L - 1)          # can only shift up to L-1
    if kk < 0:
        return out
...
rcube = np.full((m, m, K + 1), np.nan)  # [i, j, k] i leads j by k
Lmat = np.zeros((m, m), dtype=int)
print('computing pairwise TLC...', flush=True)
import time
t0 = time.time()
for i in range(m):
    xi = X[:, i]
...
# ---- best-lag (0..K) matrices ----
best_k = np.nanargmax(rcube, axis=2)          # m x m (best lag incl 0)
...
# ---- ordered-pair lead table: best strict lead k in 1..K ----
ord_rows = []
for i in range(m):
    for j in range(m):
        if i == j:
...
# ---- net-lead (direction asymmetry) per unordered pair: d(k)=r_AB(k)-r_BA(k) ----
net_rows = []
for i in range(m):
...
# ---- summary to stdout ----
r0 = np.array([[rcube[i, j, 0] for j in range(m)] for i in range(m)])
tri = np.triu(r0, 1)
tri = tri[tri > -1]
print('\n=== contemporaneous corr r0 (close, unordered pairs) ===', flush=True)
print(f'n_pairs={len(tri)} mean={tri.mean():.4f} min={tri.min():.4f} max={tri.max():.4f} '
      f'median={np.median(tri):.4f}', flush=True)
print(f'frac r0>0.99 = {(tri > 0.99).mean():.3f}', flush=True)

print('\n=== top 20 raw lead pairs (best strict lead k in 1..%d by r) ===' % K, flush=True)
print(ord_df.head(20).to_string(index=False), flush=True)

print('\n=== top 15 net-lead pairs (|r_ab(k)-r_ba(k)|, k in 1..%d) ===' % K, flush=True)
print(net_df.head(15).to_string(index=False), flush=True)
...
print(f'frac of net-lead extremes exactly at window edge k={K}: {frac_edge:.3f}', flush=True)
print(f'frac of net-lead extremes at k >= {K//2}: {frac_ge_half:.3f}', flush=True)
